...
<s:if test="#emptyList">
	<div class="alert alert-info centerTxt">
		<s:if test="#session.angemeldeterBenutzer.gruppeId == 2"><s:text name ="Ausschreibung.listen.bewerbungen.alle.bewerber" /></s:if>
		<s:elseif test="#session.angemeldeterBenutzer.gruppeId == 3"><s:text name="Ausschreibung.listen.bewerbungen.alle.ausschreiber" /></s:elseif>
		<s:elseif test="#session.angemeldeterBenutzer.gruppeId == 4"><s:text name ="Ausschreibung.listen.bewerbungen.alle.bearbeiter" /></s:elseif>
	</div>
</s:if>
<s:else>
		...
		<table class="table table-striped table-condensed">
			<thead>
				<tr>
					...
					<th>
						<s:text name="bewerbungsVorgang.status" />
					</th>
					<th>
			...
			<tbody>
				<s:iterator value="listeDaten" status="status">
					...
					<tr class="hyperlink" data-toggle="<s:url action="BewerbungsVorgang_anzeigen"><s:param name="bewerbungsVorgangId" value="id" /></s:url>">
						...
						<td><s:property value="%{getText('bewerbungsVorgang.status.' + status)}" /></td>
