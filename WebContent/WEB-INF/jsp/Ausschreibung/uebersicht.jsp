...
<s:set var="activeTab" value="'index'" />
...
<s:if test="#emptyList">
	<div class="alert alert-info centerTxt"><s:text name="Ausschreibung.listen.uebersicht" /></div>
</s:if>
<s:else>
		...
		<tbody>
			<s:iterator value="listeDaten" status="status">
				...
				<tr class="hyperlink trigger-tooltip" data-toggle="<s:url action="Ausschreibung_anzeigen"><s:param name="ausschreibungId" value="id" /></s:url>">
					...
					<td><a href="javascript:void()"><s:property value="%{name.length() > 60 ? name.substring(0, 60) + getText('global.text.ellipse') : name}" /></a></td>
					<td width="24"><span class="show-tooltip square<s:property value="%{frei > 0 ? ' green' : ' red'}" />" title="<s:text name="Ausschreibung.tabelle.plaetze.tooltip"><s:param value="frei" /><s:param value="stellenzahl" /></s:text>"></span></td>
