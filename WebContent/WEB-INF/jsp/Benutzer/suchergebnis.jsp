...
<s:set var="activeTab" value="'suche'" />
...
<s:if test="#emptyList">
	<div class="alert alert-info centerTxt">
		<s:text name="Benutzer.suchen.keineTreffer"><s:param value="suche" /></s:text>
	</div>
</s:if>
<s:else>
		...
		<tbody>
			<s:iterator value="listeDaten" status="status">
				...
				<tr class="hyperlink" data-toggle="<s:url action="%{#session.angemeldeterBenutzer.gruppeId == 1 ? 'Benutzer_aktualisieren' : 'Benutzer_anzeigen'}" includeParams="get"><s:param name="benutzerId" value="id" /></s:url>">
					<td class="hlTarget"><a href="javascript:void()"><s:property value="name" /></a></td>		
					<td class="hlTarget"><s:property value="email" /></td>
